...
describe('QuestionService', () => {
  ...
  const expected = plainToClass(QuestionEntity, {});
  ...
  describe('createOne', () => {
    it('should be return promotion entity', async () => {
      const received = await service.createOne({ ...expected });
      expect(received).toBeInstanceOf(QuestionEntity);
      expect(received).toEqual({ ...expected });
    });

    it('should be return conflict error', async () => {
      const error = new ConflictException(ErrorTypeEnum.QUESTION_NOT_FOUND);
  ...
  describe('selectAll', () => {
    it('should be return question entity', async () => {
      const received = await service.selectAll();
      expect(received).toBeInstanceOf(PaginationQuestionsDto);
    });

    it('should be return not found error', async () => {
      const error = new NotFoundException(ErrorTypeEnum.QUESTION_NOT_FOUND);
  ...
  describe('deleteOne', () => {
    ...
    it('should be return transaction error', async () => {
      const error = new NotFoundException(ErrorTypeEnum.QUESTION_NOT_FOUND);
      jest.spyOn(service, 'selectOne').mockImplementation(async () => new QuestionEntity());
      return service.deleteOne({}).catch((err) => {
        expect(err).toBeInstanceOf(NotFoundException);
        expect(err).toStrictEqual(error);
      });
