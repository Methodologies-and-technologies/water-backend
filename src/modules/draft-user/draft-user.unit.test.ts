...
describe('DraftUserService', () => {
  let service: DraftUserService;

  const owner = plainToClass(UserEntity, { id: '067f2f3e-b936-4029-93d6-b2f58ae4f489' });
  const expected = plainToClass(DraftUserEntity, {
  ...
  beforeAll(async () => {
    ...
    service = module.get<DraftUserService>(DraftUserService);
  });
  ...
  describe('createOne', () => {
    it('should be return draft-user entity', async () => {
      const received = await service.register({ ...expected });
      expect(received).toBeInstanceOf(DraftUserEntity);
  ...
  describe('selectAll', () => {
    it('should be return draft-user entity', async () => {
      ...
      expect(received).toBeInstanceOf(PaginationDraftUsersDto);
    });
  ...
  describe('deleteOne', () => {
    ...
    it('should be return transaction error', async () => {
      const error = new NotFoundException(ErrorTypeEnum.FULLNAME_ALREADY_TAKEN);
      jest.spyOn(service, 'selectOne').mockImplementation(async () => new DraftUserEntity());
      return service.deleteOne({}).catch((err) => {
        expect(err).toBeInstanceOf(NotFoundException);
        expect(err).toStrictEqual(error);
      });
