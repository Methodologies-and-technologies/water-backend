...
describe('PromotionService', () => {
  ...
  const expected = plainToClass(PromotionEntity, {});
  ...
  describe('createOne', () => {
    it('should be return promotion entity', async () => {
      ...
      expect(received).toBeInstanceOf(PromotionEntity);
      expect(received).toEqual({ ...expected });
    });

    it('should be return conflict error', async () => {
      const error = new ConflictException(ErrorTypeEnum.PROMOTION_NOT_FOUND);
  ...
  describe('selectAll', () => {
    it('should be return promotion entity', async () => {
      ...
      expect(received).toBeInstanceOf(PaginationPromotionsDto);
    });

    it('should be return not found error', async () => {
      const error = new NotFoundException(ErrorTypeEnum.PROMOTION_NOT_FOUND);
  ...
  describe('deleteOne', () => {
    ...
    it('should be return transaction error', async () => {
      const error = new NotFoundException(ErrorTypeEnum.PROMOTION_NOT_FOUND);
      jest.spyOn(service, 'selectOne').mockImplementation(async () => new PromotionEntity());
      return service.deleteOne({}).catch((err) => {
        expect(err).toBeInstanceOf(NotFoundException);
        expect(err).toStrictEqual(error);
      });
